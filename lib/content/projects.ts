...
export const projects: Project[] = [
  ...
  },
  {
    id: "agenthub",
    title: "AgentHub",
    tagline: "Enterprise AI Agent Management Platform",
    category: "ai",
    categoryLabel: "AI SaaS Platform",
    badges: ["MULTI-TENANT", "NO-CODE", "ENTERPRISE"],
    description:
      "Enterprise SaaS platform for building, deploying, and monitoring production AI agents across multi-tenant organisations. A visual no-code flow builder (React Flow canvas) lets teams compose agents from reusable components — LLM nodes, tool calls, conditional branches — and test them in a live playground. An AI gateway normalises requests across multiple LLM providers, while a knowledge base module powers per-agent RAG pipelines and an observability dashboard tracks agent performance in real time.",
    highlights: [
      "Visual agent flow builder on a React Flow canvas — drag-and-drop composition of LLM nodes, tool-call components, and conditional branches; flows versioned and persisted via a backend API. Reduces time-to-agent from weeks to hours for non-technical teams.",
      "AI gateway layer normalises model requests across multiple LLM providers and platforms — teams swap underlying models without changing agent logic, eliminating vendor lock-in and reducing migration cost.",
      "Knowledge base module enables per-agent RAG pipelines — organisations attach document collections to specific agent flows, giving agents accurate, org-specific context without fine-tuning.",
      "Real-time agent execution streaming via SSE with an in-browser playground for iterating on prompts and inspecting agent steps before promoting to production.",
      "Multi-tenant org model with team and department management, RBAC, prompt gallery, leaderboard, and a real-time observability dashboard tracking agent usage and performance across the platform.",
    ],
    techStack: ["Next.js", "TypeScript", "React Flow", "MUI", "Monaco Editor", "Zustand", "next-auth", "next-intl", "Docker"],
    links: {},
  },
  {
    id: "sagenest",
    title: "SageNest",
    tagline: "Expert Mentoring Marketplace Platform",
    category: "fullstack",
    categoryLabel: "EdTech & Marketplace",
    badges: ["MARKETPLACE", "REAL-TIME", "PAYMENTS"],
    description:
      "Full-stack expert mentoring marketplace connecting learners with domain experts for 1:1 sessions, structured learning programs, and live webinars. Learners discover experts, book time slots, and pay via Razorpay — with instant payment confirmation and enrolment notifications pushed over STOMP WebSocket. In-session communication runs via CometChat (real-time messaging) and Daily.co/Zoom (video). A Spring Boot microservice handles registration, OAuth2 (Google + LinkedIn), bookings, payments, session feedback, and email — backed by PostgreSQL with Liquibase-managed migrations.",
    highlights: [
      "End-to-end booking lifecycle — expert discovery, scheduling, Razorpay payment, video call (Daily.co + Zoom), and post-session feedback — creating a seamless, accountable learning experience that converts informal mentoring into a scalable marketplace.",
      "STOMP WebSocket layer delivers instant payment confirmations and program enrolment updates to the frontend — users get real-time feedback without polling, keeping drop-off rates low at the critical payment moment.",
      "CometChat SDK integration provides persistent real-time messaging between learners and experts, maintained across sessions with unread badges and notification handling.",
      "Spring Boot onboarding microservice with OAuth2 (Google + LinkedIn), Spring Security, Liquibase DB migrations, and WireMock-backed integration tests covering registration, bookings, payments, and feedback service contracts.",
      "LinkedIn OAuth profile sync auto-populates expert profiles with experience and skills on sign-up — reducing onboarding friction for supply-side users and improving marketplace data quality from day one.",
    ],
    techStack: ["Next.js", "TypeScript", "Spring Boot", "Java", "Redux Toolkit", "CometChat", "WebSocket", "Razorpay", "PostgreSQL", "OAuth2"],
    links: {},
  },
  {
    id: "cloudops-hub",
